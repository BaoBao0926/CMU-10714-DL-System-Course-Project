...
# ------------------------------------------------------
# 1. Load torchvision ResNet50 + preprocess + label map
# ------------------------------------------------------
def load_torch_resnet50():
...
# ------------------------------------------------------
# 2. Convert ResNet50 to Needle
# ------------------------------------------------------
def convert_resnet50_to_needle(torch_model, device, dtype):
...
# ------------------------------------------------------
# 3. Run inference on a single image (torch or needle)
# ------------------------------------------------------
...
# ------------------------------------------------------
# 4. Evaluate on 20 images
# ------------------------------------------------------
def evaluate_on_images(torch_model, needle_model, preprocess, categories, device, dtype):
    image_dir = "images"
    ...
    for idx, name in enumerate(files, 1):
        ...
        if torch_pred == needle_pred:
            needle_correct += 1
        torch_correct += 1   # PyTorch always correct relative to itself
...
if __name__ == "__main__":
    print("\n===== Step 1: Load PyTorch ResNet50 =====")
    torch_model, preprocess, categories = load_torch_resnet50()

    print("\n===== Step 2: Convert to Needle =====")
    device = ndl.cpu()
    dtype = "float32"

    needle_model = convert_resnet50_to_needle(
        torch_model, device, dtype
    )

    print("\n===== Step 3: Evaluate on 20 images =====")
    evaluate_on_images(
        torch_model, needle_model,
