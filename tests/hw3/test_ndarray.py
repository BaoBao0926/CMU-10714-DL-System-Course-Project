...
_DEVICES = [
    nd.cpu(),
    pytest.param(
        nd.hip(), marks=pytest.mark.skipif(not nd.hip().enabled(), reason="No AMD GPU")
    ),
]
...
def RandC(*shape, entropy=1):
    if nd.hip().enabled():
        return Rand(*shape, device=nd.hip(), entropy=2)
    else:
...
def submit_ndarray_hip_compact_setitem():
...
def submit_ndarray_hip_reductions():
