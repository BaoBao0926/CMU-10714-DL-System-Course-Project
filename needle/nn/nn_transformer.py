...
class AttentionLayer(Module):
    ...
    def forward(
        ...
        q = q.reshape((batch_size,queries_len,self.num_head,self.dim_head)).transpose((0,2,1,3))
        k = k.reshape((batch_size,keys_values_len,self.num_head,self.dim_head)).transpose((0,2,1,3))
        v = v.reshape((batch_size,keys_values_len,self.num_head,self.dim_head)).transpose((0,2,1,3))
        result,_ = self.attn(q,k,v)
        result = result.transpose((0,2,1,3)).reshape((batch_size*keys_values_len,self.num_head * self.dim_head))
...
class TransformerLayer(Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.prenorm = LayerNorm1d(q_features,device=device,dtype=dtype)
        self.linear_in = Linear(q_features,hidden_size,device=device,dtype=dtype)
        self.linear_out = Linear(hidden_size,q_features,device=device,dtype=dtype)
        self.relu = ReLU()

        # self.attn_layer = Sequential(
        #     AttentionLayer(q_features,num_head,dim_head,dropout=dropout,
    ...
    def forward(
        ...
        # ffn
        # x->hidden
        hidden = x.reshape((batch_size * seq_len,x_dim))
        hidden = self.linear_in(self.prenorm(hidden))
        hidden = self.dropout(self.relu(hidden))
        # hidden -> y
        hidden = self.dropout(self.linear_out(hidden))
        hidden = hidden.reshape((batch_size,seq_len,x_dim))
        x = x+hidden
        ### END YOUR SOLUTION
...
class Transformer(Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.transformer = Sequential(*blocks)
        ### END YOUR SOLUTION
    ...
    def forward(
        ...
        pos_ids = ndarray.NDArray(range(seq_len),device=self.device).reshape((1,seq_len))
        pos_ids = pos_ids.broadcast_to((batch_size,seq_len))
        pos_embed = Tensor(pos_ids,device=self.device,dtype=self.dtype,requires_grad=False)
