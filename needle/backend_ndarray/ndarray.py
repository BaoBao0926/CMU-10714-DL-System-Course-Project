...
class NDArray:
    ...
    def reshape(self, new_shape: tuple[int, ...]) -> "NDArray":
        ...
        if prod(new_shape)!= prod(self.shape):
            raise ValueError("product of new shape is not equal to product of original shape!")
        if not self.is_compact():
            raise ValueError("this array is not compact!")
        #new_stride = tuple(prod(new_shape[i+1:]) for i in range(len(new_shape)))
        #print(new_stride)
        return self.make(new_shape,strides=self.compact_strides(new_shape),device=self._device,handle=self._handle,offset=self._offset)
        ### END YOUR SOLUTION
    ...
    def permute(self, new_axes: tuple[int, ...]) -> "NDArray":
        ...
        return self.make(new_shape,strides=new_strides,device=self._device,handle=self._handle,offset=self._offset)
        ### END YOUR SOLUTION
    ...
    def broadcast_to(self, new_shape: tuple[int, ...]) -> "NDArray":
        ...
        strides = list((0,) * padding + self._strides)
        for i in range(len(new_shape)):
            if not (new_shape[i]==shape[i] or shape[i] == 1):
                raise AssertionError("incompatible shape")
            if(shape[i] != new_shape[i]):
                strides[i] = 0

        # new_strides = []
        # for i in range(len(new_shape)):
        #     if(self._shape[i] != new_shape[i] and self._shape[i] != 1):
        #         raise AssertionError("new_shape[i] != shape[i] for all i where shape[i] != 1")
        #     if(self._shape[i] != new_shape[i]):
        #         new_strides.append(0)
        #     else:
        #         new_strides.append(self._strides[i])
        
        return self.make(new_shape,strides=tuple(strides),device=self._device,handle=self._handle,offset=self._offset)
        ### END YOUR SOLUTION
    ...
    def __getitem__(self, idxs: int | slice | tuple[int | slice, ...]) -> "NDArray":
        ...
        ### BEGIN YOUR SOLUTION
        new_shape = [(sl.stop-sl.start+sl.step-1)//sl.step for sl in slices]
        # offset = sum([sl.start * st for sl,st in zip(slices,self._strides)])
        new_strides = tuple([sl.step * st for sl,st in zip(slices,self._strides)])
        offset = 0
        for sl,st in zip(slices,self._strides):
            offset += sl.start* st
        # new_shape = []
        # new_strides = []
        # offset = self._offset
        ...
        #     dim_stride = step * self._strides[i]
        #     offset += start * self._strides[i]
        #     new_shape.append(dim_len)
        #     new_strides.append(dim_stride)
        return self.make(new_shape,new_strides,device=self._device,handle=self._handle,offset=offset)
        ### END YOUR SOLUTION
    ...
    def sum(self, axis: int | tuple[int, ...] | list[int] | None = None, keepdims: bool = False) -> "NDArray":
        ...
        if axis == None and keepdims==True: # if the dimemsion should be kept, but we like to find the global sum
            new_shape = [out.shape[0] for i in range(len(self.shape))]
    ...
    def flip(self, axes: tuple[int, ...]) -> "NDArray":
        ...
        # if negative axis is applied, normalize negative axis
        axes = tuple(a if a >=0 else self.ndim+a for a in axes)
        new_strides = tuple(-self.strides[ax] if ax in axes else self.strides[ax] for ax in range(self.ndim))
        new_offset = self._offset
        for ax in axes:
            new_offset += (self._shape[ax] -1) * self._strides[ax]
            #new_strides[ax] = -new_strides[ax]
        flipped_view = self.make(shape=self._shape,strides=new_strides,offset=new_offset,device=self._device,handle=self._handle) 
        return flipped_view.compact() # immediately compact array after making a new view
    ...
    def pad(self, axes: tuple[tuple[int, int], ...],constant_value=0) -> "NDArray":
        ...
        for i,(before,after) in enumerate(axes):
            new_dim = before+ ori_shape[i] + after
