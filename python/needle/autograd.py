...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
        if not node.requires_grad:
            continue
        #### Just for debugging###
        # node_grad = sum_node_list(node_to_output_grads_list.get(node,[]))
        # node.grad = node_grad
        # if count==31:
        #     print("HAHA, I am in count=31!")
        # count+=1
        ############################
        node.grad = sum_node_list(node_to_output_grads_list.get(node,[])) # sum partial adjoints
        if node.is_leaf(): # no op for this node, this node may be a input node
            continue
        # compute gradient for each input
        child_grads = node.op.gradient_as_tuple(node.grad,node)
        for child_node,child_node_grad in zip(node.inputs,child_grads):
            if not child_node.requires_grad:
                continue
            # if no key name chile_node, set to [], else append child_node_grad
            node_to_output_grads_list.setdefault(child_node,[]).append(child_node_grad)
...
def topo_sort_dfs(node, visited, topo_order):
    """Post-order DFS"""
    ### BEGIN YOUR SOLUTION
    visited.add(node) # if the node is visited, mark this node as visited
